...
export async function POST(request: NextRequest) {
  const { email, interestedIn, investmentAmount, name, surname, phone } = await request.json();

  try {
    const response = await subscribeToMailchimp(email, interestedIn, investmentAmount, name, surname, phone);
